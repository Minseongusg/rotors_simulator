...
void GazeboMavlinkInterface::OnUpdate(const common::UpdateInfo& /*_info*/) {
  ...
  //send gps
  ignition::math::Pose3d T_W_I = model_->GetWorldPose(); //TODO(burrimi): Check tf.
...
void GazeboMavlinkInterface::ImuCallback(ImuPtr& imu_message) {
  ...
  */
  ignition::math::Quaternion q_br(0, 1, 0, 0);
  ...
  */
  ignition::math::Quaternion q_ng(0, 0.70711, 0.70711, 0);

  ignition::math::Quaternion q_gb = q_gr*q_br.GetInverse();
  ignition::math::Quaternion q_nb = q_ng*q_gb;
  ...
  ignition::math::Quaternion q_dn(0.0, 0.0, declination);
